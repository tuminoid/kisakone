...
<table class="results" id="result_table">
    {foreach from=$results item=group key=groupind}
        ...
        {foreach from=$group item=result key=memberInd}
            <tr class="resultrow player_{$result.playerId}">
                ...
                {/foreach}
                <td>
                    <input class="penalty" type="text" id="r{$result.PlayerId}_p" size="2" name="r{$result.PlayerId}_p" value="{$result.Penalty}" />
                </td>
                <td>
                    <input type="text" id="r{$result.PlayerId}_sd" size="2" name="r{$result.PlayerId}_sd" value="{$result.SuddenDeath}" />
                </td>
    ...
    function searchGoto(e) {
        ...
        if (hilightedName)
            $(hilightedName).removeClass('name_hilight');
    ...
    function isHeadingRow(r) {
        ...
        while (c != null) {
            if (c.tagName) {
                if (c.tagName.match(/^td$/i))
                    return false;
                if (c.tagName.match(/^th$/i))
                    return true;
            }
        ...
        if (field) {
            ...
            if (diff) {
                ...
                if (data.hole != "sd") {
                    ...
                    if (pmfield.firstChild)
                        pmfield.removeChild(pmfield.firstChild);
