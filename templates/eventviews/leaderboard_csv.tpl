 ...
 *
 * Leaderboard CSV for importing into pdga.com and PDGA TD-report
 *
 * --
 *
...
{$class}{""|indent:1:"\t"}{$result.Standing}{""|indent:1:"\t"}{$result.FirstName|escape}{""|indent:1:"\t"}{$result.LastName|escape}{""|indent:1:"\t"}{$result.PDGANumber|escape}{""|indent:1:"\t"}{foreach from=$rounds item=round key=index}{assign var=roundid value=$round->id}{assign var=rresult value=$result.Results.$roundid.Total}{assign var=roundresult value=$rresult}{if $result.Results.$roundid.DNF}{assign var=roundresult value=999}{/if}{$roundresult}{""|indent:1:"\t"}{/foreach}{if $includePoints}{assign var=tournamentPoints value=$result.TournamentPoints}{if !$tournamentPoints}{assign var=tournamentPoints value=0}{/if}{math equation="x/10" x=$tournamentPoints}{""|indent:1:"\t"}{/if}
{/foreach}
{/foreach}
{/if}
</textarea>
