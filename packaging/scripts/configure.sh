...
DB_FILE="/kisakone/kisakone.sql.backup"
if [ -e ${DB_FILE} ]; then
  echo "Restoring backup SQL from: ${DB_FILE}"
  mysql -u root --password=pass < ${DB_FILE}
  ...
  DB=$(grep "Current Database" ${DB_FILE} | grep kisakone | grep -v "next" | grep -v "test" | head -1 | cut -f2 -d'`' | cut -f1 -d'`')
  if [ "$DB" != "" ]; then
