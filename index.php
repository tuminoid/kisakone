...
// we'll fix that here
if (!is_array($pagename)) $pagename = array($pagename);
...
if (GetMainMenuSelection() == 'administration') language_include('admin');
...
function gate_stripSlashesIfNecessary()
{
         ...
         foreach ($_GET as $k => $v) $_GET[$k] = recursive_stripslashes($v);
         foreach ($_REQUEST as $k => $v) $_REQUEST[$k] = recursive_stripslashes($v);
         foreach ($_COOKIE as $k => $v) $_COOKIE[$k] = recursive_stripslashes($v);
